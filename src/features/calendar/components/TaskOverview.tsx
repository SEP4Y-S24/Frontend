  ...
  return (
    <div className="flex items-center justify-between space-x-3 px-3 py-1 my-2 bg-whiteHover hover:bg-background rounded">
      <div>
        ...
        <Heading
          text={"Deadline: " + deadlineDate + " at: " + deadlineTime}
          type={"heading5"}
        />
      </div>
      <div>
        <Badge
          text={status.name}
          styleType={getStyleTypeByStatus(status.name)}
          isFilled={false}
        />
      </div>
...
export const TaskOverview: React.FC<TaskOverviewProps> = ({
  ...
  return (
    <ContentInnerContainer className="flex-1 h-16 md:h-auto bg-white">
      ...
      <Badge
        styleType={"warning"}
        isFilled={isNotStarted}
        text={"Not started"}
        onClick={handleFilterIsNotStarted}
      />
      <Badge
        styleType={"info"}
        isFilled={isInProgress}
        text={"In progress"}
        onClick={handleFilterIsInProgress}
      />
      <Badge
        styleType={"success"}
        isFilled={isDone}
        text={"Done"}
        onClick={handleFilterIsDone}
      />
      {tasks ? (
        tasks
