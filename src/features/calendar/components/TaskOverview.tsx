...
export const TaskOverview: React.FC<TaskOverviewProps> = ({setSelectedTask, tasks, setTasks, setMode}) => {
    ...
    const filteredTasks = useMemo(() => {
        ...
        return tasks.filter(task => {
            return filters[task.status.name];
        });
    }, [tasks, filters]);
    ...
    return (
        <ContentInnerContainer className="flex-1 h-16 md:h-auto bg-white">
            ...
            {Object.keys(filters).map((filter, index) => (
                <Badge
                    key={index}
                    styleType={getStyleTypeByStatus(filter)} // You may need to define this function at the top
                    isFilled={filters[filter]}
                    text={filter}
            ...
            <PaginationRounded
                ...
                pages={Math.ceil(filteredTasks.length / tasksPerPage)}/>
